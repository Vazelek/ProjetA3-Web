...
if len(sys.argv) != 9:
    ...
    if len(sys.argv) == 2:
        if not (sys.argv[1] == "-h" or sys.argv[1] == "--help"):
            ...
            print("Requis : 8, donnés : " + str(len(sys.argv) - 1))
            print("")
    else:
        print("Erreur : Nombre incorrect d'arguments")
        print("Requis : 8, donnés : " + str(len(sys.argv) - 1))
...
if data_csv["latitude"][0] < -90 or data_csv["latitude"][0] > 90 or \
    ...
    print("")
    print("Format de la commande :")
    print("\t>>> python script3_classification.py <latitude> <longitude> <descre_athmo> <descr_lum> <descr_etat_surf> <age> <descr_dispo_secu> <methode>")
    print("")
    print("<latitude> :\t\t{float}   [-90, 90]")
    print("<longitude> :\t\t{float}   [-180, 180]")
    print("<descr_athmo> :\t\t{int}     [1, 9]")
...
if method == "SVM":
    if os.path.isfile("../gen_python/variables/svm"):
        file = open("../gen_python/variables/svm", "rb")
    ...
    if os.path.isfile("../gen_python/variables/rf"):
        file = open("../gen_python/variables/rf", "rb")
        model = pickle.load(file)
        file.close()
    else:
    ...
    if os.path.isfile("../gen_python/variables/mlp"):
        file = open("../gen_python/variables/mlp", "rb")
        model = pickle.load(file)
        file.close()
    else:
